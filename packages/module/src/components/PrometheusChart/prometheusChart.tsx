...
import { Chart, ChartAxis, ChartArea, ChartGroup, ChartLegend, ChartLine, ChartStack, ChartThemeColor, ChartVoronoiContainer, ChartThemeVariant, getCustomTheme, ChartThreshold } from '@patternfly/react-charts';
import { useRefWidth } from '../../utils/ref-width-hook';
import { dateTimeFormatterWithSeconds, parsePrometheusDuration, timeFormatter } from '../../utils/datetime';
...
const getXDomain = (endTime: number, span: number): [number, number] => [endTime - span, endTime];
...
const Tooltip: React.FC<TooltipProps> = ({ activePoints, center, height, style, width, x, threshold, thresholdText }) => {
            ...
            <div className="prometheus-chart__tooltip">
              <div className="prometheus-chart__tooltip-group">
                <div className="prometheus-chart__tooltip-time">
                  {threshold ?
                  (thresholdText ? `${thresholdText}: ${formatValue(threshold)}` : `Limit: ${formatValue(threshold)}`) : dateTimeFormatterWithSeconds.format(time)}
                </div>
              </div>
              {allSeries.map((s, i) => (
                <div className="prometheus-chart__tooltip-group" key={i}>
                  <div className="prometheus-chart__series-btn" style={{ backgroundColor: s.color }} />
                  <div className="nowrap truncate">{s.name}</div>
                  <div className="prometheus-chart__tooltip-value">{formatValue(s.value)}</div>
                </div>
              ))}
            </div>
          </div>
    ...
    const tick = () => {
        ...
        return Promise.all(allPromises)
            .then((responses: PrometheusResponse[]) => {
                ...
                const newGraphData = _.map<any, Series[]>(newResults, (result: PrometheusResult[]) => {
                    return _.map(
                        result,
...
const PrometheusChartGraph: React.FunctionComponent<PrometheusChartGraphProps> = ({
    ...
    const graphContainer = (
        <ChartVoronoiContainer
            activateData={false}
            labels={() => ' '}
            labelComponent={<Tooltip threshold={thresholdData?.threshold} thresholdText={thresholdData?.thresholdText}/>}
            mouseFollowTooltips
            voronoiDimension="x"
            voronoiPadding={0}
        />
    ...
    return (
        <Chart
            ...
            legendComponent={showLegend ? (<ChartLegend
                data={legendData}
                itemsPerRow={4}
        ...
        >
            ...
            {!_.isEmpty(thresholdData?.data) && (<ChartThreshold
                key="chart-threshold"
