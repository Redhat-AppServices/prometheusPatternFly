...
import { Chart, ChartAxis, ChartArea, ChartGroup, ChartLegend, ChartLine, ChartStack, ChartThemeColor, ChartVoronoiContainer, ChartThemeVariant, getCustomTheme } from '@patternfly/react-charts';
...
const Tooltip: React.FC<TooltipProps> = ({ activePoints, center, height, style, width, x }) => {
...
export const PrometheusChart: React.FunctionComponent<PrometheusChartProps> = ({
    ...
    const tick = () => {
        const allPromises = _.map(prometheusURLs, (prometheusURL) => 
            _.isEmpty(prometheusURL)
                ? Promise.resolve()
                : fetch(prometheusURL)
    ...
    const LegendContainer = ({ children }: { children?: React.ReactNode }) => {
        ...
        return (
            <foreignObject height={75} width="100%" y={245}>
                <div className="prometheus-chart__legend-wrap">
